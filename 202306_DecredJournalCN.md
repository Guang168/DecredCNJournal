...
- [Upgrade and Vote on New Consensus Changes!](#upgrade-and-vote-on-new-consensus-changes)
- [Core Software v1.8.0 Release](#core-software-v180-release)
- [DCRDEX v0.6.2 Release](#dcrdex-v062-release)
...
The voting has started on July 4 and will run until around August 1 (block [786,303](https://dcrdata.decred.org/block/786303)).
...
If DCP-11 is approved, after the 1-month [upgrade period](https://docs.decred.org/governance/consensus-rule-voting/overview/) the difficulty will be reset to a [hard-coded value](https://github.com/decred/dcrd/blob/a13ca3ace61d28678e1889b9a6303dd88c3dae98/chaincfg/mainnetparams.go#L115) which equates to an initial target hashrate of about 1.45 TH/s. This number does matter much though because it will very quickly adjust due to the new difficulty algorithm.
...
- Ensured that the wallet can correctly validate blocks mined with either the [original BLAKE-256 or the upcoming BLAKE3](https://github.com/decred/dcrwallet/pull/2240) proof of work hashes, and updated the vote version.
- Mixed ticket purchases now have a [random 20-60 second delay](https://github.com/decred/dcrwallet/pull/2244) before the wallet publishes each transaction to the mempool. This trickle effect will improve privacy by making it much harder for anyone surveilling the mempool to link tickets together based on published timestamp.
...
- Bumped the major module version of the in-dev [dcrwallet module](https://github.com/decred/dcrwallet/pull/2248) to v4 to prepare for future changes.
- Removed a deprecated option from the internal wallet config which enabled automatic [`AddressReuse`](https://github.com/decred/dcrwallet/pull/2258) in some functions like the auto ticket buyer. The option was originally [deprecated](https://github.com/decred/dcrwallet/pull/1442) in v1.5 and [removed](https://github.com/decred/dcrwallet/pull/1676) from the main package in v1.6 because reusing addresses is bad for privacy. The wallet stopped using the `AddressReuse` option, but it was left deprecated in the internal package to preserve code compatibility at the time.
...
- Updated DCRDEX module to the [v0.6.1 release](https://github.com/decred/decrediton/pull/3871).
...
- Updated [DEX module to v0.6.2](https://github.com/decred/decrediton/pull/3882), which is required to follow the Decred 1.8.0 hardforks due to consensus changes. In addition, this fixes the "response buffer too short" error reported by some users.
...
- Updated to [Electron v23](https://github.com/decred/decrediton/pull/3884) from v18. Among other things, this allows Decrediton to communicate with USB devices, which is required for some hardware wallets like Ledger. Since [WebUSB](https://en.wikipedia.org/wiki/WebUSB) is built-in to the newer versions of Electron, it means devs don't have to use external dependencies, which is good for security.
...
- Internal dev updates: updated [linters](https://github.com/decred/vspd/pull/384), refactored [database encoding](https://github.com/decred/vspd/pull/384), updated to use latest [dcrd](https://github.com/decred/vspd/pull/387) and [dcrwallet](https://github.com/decred/vspd/pull/388) [modules](https://github.com/decred/vspd/pull/392) in the client and in the [server](https://github.com/decred/vspd/pull/389), updated Gin [web framework](https://github.com/decred/vspd/pull/391).
...
- Significantly reduced the [initial deposit required for bonds](https://github.com/decred/dcrdex/pull/2392), from 0.056 BTC ($1,500) to 0.0035 BTC ($100). This will make it easier to onboard new users. Note that bonds can be funded with DCR too.
...
- Optimized loading of [static GUI assets](https://github.com/decred/dcrdex/pull/2398) in the browser. CSS, JavaScript, images, and font files will now load faster.
...
- Added [release notes](https://github.com/decred/dcrdex/pull/2394) for v0.6.1.
...
- Implemented several low-level primitives and a [test harness](https://github.com/decred/dcrdex/pull/2399) to aid in developing Polygon support. The harness was able to sync balances and send funds on simnet and testnet. Existing Ethereum code has been refactored to be more generic and reusable by Polygon code.
...
- Updated simnet blockchain with [new proof of work](https://github.com/decred/dcrdex/pull/2358).
- Added scripts to automatically [update cache busters](https://github.com/decred/dcrdex/pull/2363).
- Enabled [caching](https://github.com/decred/dcrdex/pull/2390) of Go build outputs produced by GitHub Actions. This avoids unnecessary recompilation and speeds up builds and tests, which run for every pull request. As a result, it takes less time to see if a change being developed breaks any tests.

![](../img/202306.04.516.png)

_Image: Improved intro to bonds in DCRDEX_

![](../img/202306.05.933.png)

_Image: Wallet types supported by DCRDEX_
...
- Multiple updates for command-line apps usage in [v1.8.0 release](https://github.com/decred/dcrdocs/pull/1223).
...
- Refined [chat list UX](https://github.com/companyzero/bisonrelay/pull/292).
...
- Configured the embedded dcrlnd to [not include IP or hostname](https://github.com/companyzero/bisonrelay/pull/267) in generated TLS certificates. This is needed to make the Android version compile.
...
- Moved and adapted some [desktop code](https://github.com/companyzero/bisonrelay/pull/267) to a shared location to enable reuse on mobile.
...
- Updated Decred and third party [Go dependencies](https://github.com/companyzero/bisonrelay/pull/268).
- Improved GitHub [build config](https://github.com/companyzero/bisonrelay/pull/280), automated more of the [release process](https://github.com/companyzero/bisonrelay/pull/274), and [cleaned up](https://github.com/companyzero/bisonrelay/pull/279) some code.
...
- gominer was [updated](https://github.com/decred/gominer/pull/193) for latest Decred modules, linters, and GitHub Actions build config.
...
- A [proposal](https://proposals.decred.org/record/552c87e) to fund development of the Decred.club website with Chinese language content and grow a community around it for $2,400.
...
- New VSP at [vote.dcr-swiss.ch](https://www.vote.dcr-swiss.ch/) has started their testnet and mainnet testing to get added to the [VSP list](https://decred.org/vsp/).
...
- Binance and CZ have been [sued by U.S. SEC](https://www.coindesk.com/policy/2023/06/05/sec-sues-crypto-exchange-binance-ceo-changpeng-zhao/) for violating securities laws. The charges are similar to those filed against Binance by [CFTC in March](202303.md#relevant-external). The news has triggered massive [withdrawals](https://www.reuters.com/technology/crypto-exchange-binance-hit-by-outflows-780-mln-last-24-hours-nansen-2023-06-06/) from Binance and Binance.US. DCR is still not in the [updated list of 50+ assets](https://cointelegraph.com/news/sec-labels-61-cryptocurrencies-securities-after-binance-suit) mentioned as "securities" by the SEC at different points in time. Binance is still the biggest market for DCR if the reported traded volume can be trusted.
...
- Likes: 1,070
- Re-tweets: 256
- Social media followers across all platforms and accounts (including [@DecredSociety](https://twitter.com/DecredSociety)): 1,410
...
- [Advanced trading options for DCRDEX](https://www.youtube.com/watch?v=yqqzKFmBA8E) by @phoenixgreen
...
- [Crypto Unity - Building decentralised bonds - State of the Market](https://www.youtube.com/watch?v=CPQpP7K6EN8) by @phoenixgreen, @Exitus, and @Tivra featuring [JohnnyLaw2021](https://twitter.com/JohnnyLaw2021), [ZecHub](https://twitter.com/ZecHub), and [LTCUnderground](https://twitter.com/LTCUnderground) - also as a [podcast](https://podcasters.spotify.com/pod/show/cypherpunktimes/episodes/Crypto-Unity---Building-Decentralised-Bonds---State-of-the-market-e25n9na)

**Audio:**

- [Crypto under siege?](https://twitter.com/i/spaces/1OyJAVgRnEOxb) - Twitter Space with @Tivra and [BawdyAnarchist_](https://twitter.com/BawdyAnarchist_) discussing the cryptocurrency situation - also on [Spotify](https://podcasters.spotify.com/pod/show/cypherpunktimes/episodes/Crypto-Under-Siege-e260ibe)

**Art and fun:**

- [Leading the DAO pack](https://www.cypherpunktimes.com/leading-the-dao-pack/) by @OfficialCryptos
- [Decred's innovation train](https://www.cypherpunktimes.com/decred-innovation-train/) by @OfficialCryptos
- [IQ bell curve](https://twitter.com/exitusdcr/status/1671540620328828930) by @Exitus
- [The only thing that may upset a strong man](https://twitter.com/exitusdcr/status/1665459255682519042) by @Exitus
- A bunch of hilarious [old people memes](https://matrix.to/#/!lDZCzVQjFoJsXMPkvr:decred.org/$hpdZUQThNeIg6FphVY1Vk3Gwr2qWYYVahrxRdZMxexU) has been posted on Matrix and Bison Relay's #trading chat around June 28

**Translations:**

- Decred Journal March-May got a total of 3 new [translations](https://xaur.github.io/decred-news/) to Arabic (@arij, @abdulrahman4), Chinese (@Dominic) and Polish (@kozel). Thank you folks!

**Discussions:**

- Challenging for other projects to compete with Decred's [initial sync time](https://twitter.com/decredproject/status/1673431473234366464)
- Which coins to integrate next in DCRDEX - a survey on [Twitter](https://twitter.com/decredproject/status/1668408872107687936) and on [Reddit](https://www.reddit.com/r/decred/comments/148de6k/which_coin_should_be_next_on_our_dcrdex/)
- [DCRDEX announcement](https://www.reddit.com/r/litecoin/comments/13xonle/litecoin_atomicswaps_trade_litecoin_p2p_with_no/) on r/litecoin got 90+ upvotes
- Zclassic (ZCL) [teased](https://twitter.com/ZclassicCoin/status/1668873387907637249) Twitter about a possible DEX integration
- Rhett Creighton (founder of [Zclassic](https://zclassic.org/) and [Bitcoin Private](https://www.investopedia.com/terms/b/bitcoin-private-btcp.asp)) made a [livestream](https://twitter.com/rhett_creighton/status/1668133884779974656) of him exploring cross-chain atomic swaps and DCRDEX. That video informed several [UX improvements](https://matrix.to/#/!mlRZqBtfWHrcmgdTWB:decred.org/$lzWuh7yk-jxorxgLyppFIInMSZmvfXQVM8du3tgtBs4) to DEX app UI and documentation.

**Selected non-Decred articles** posted on Cypherpunk Times:

- [BRC-20; Here to stay or fade?](https://www.cypherpunktimes.com/brc-20-here-to-stay-or-fade/) by @BlockchainJew - also covers the new Taproot Assets Protocol
...
_Image: Leading the DAO pack by @OfficialCryptos_
...
_Image: Market Valuations (USD) based on Decred-specific [Staked Realized Value](https://bochinchero.medium.com/decred-on-chain-staked-realised-value-444ab5a146d8) metric from @bochinchero_

![](../img/202306.22.768.jpg)

_Image: Decred-specific SASRV/RV Ratio metric by @bochinchero provides a lagging indicator of the direction that the market is trending_
...
_Image: Realized Value minus Staked Realized Value by @bochinchero_
